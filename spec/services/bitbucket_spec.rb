...
describe Service::Bitbucket do
  ...
  describe 'receive_verification' do
    before do
      @config = { :project_url => 'https://bitbucket.org/user_name/project_name' }
  ...
  describe 'receive_issue_impact_change' do
    ...
    it 'should succeed upon successful api response' do
      test = Faraday.new do |builder|
        builder.adapter :test do |stub|
          stub.post('/api/1.0/repositories/user_name/project_name/issues') { [201, {}, "{\"id\":\"foo\"}"] }
          stub.get('/api/1.0/repositories/user_name/project_name/issues') { [200, {}, "{\"id\":12345}"] }
        end
      end
    ...
    it 'should fail upon unsuccessful api response' do
      test = Faraday.new do |builder|
        builder.adapter :test do |stub|
          stub.post('/api/1.0/repositories/user_name/project_name/issues') { [500, {}, "{\"id\":\"foo\"}"] }
          stub.get('/api/1.0/repositories/user_name/project_name/issues') { [200, {}, "{\"id\":12345}"] }
        end
      end
